...
export default async function GestionViajes({
  ...
  const statTotal = todosLosViajes.length;
  const statEnRuta = todosLosViajes.filter(v => v.estado === 'En camino').length;
  const statPendientes = todosLosViajes.filter(v => v.estado === 'Programado').length;
  const statCancelados = todosLosViajes.filter(v => v.estado === 'Cancelado').length;
      ...
      <aside className="w-[260px] bg-[#FFFFFF] border-r border-[#D8DADC] hidden lg:flex flex-col justify-between shrink-0 h-full">
        <div>
          ...
          <nav className="p-4 flex flex-col gap-2">
            ...
            <Link href="/viajes" className="flex items-center gap-3 px-3 py-2.5 bg-[#0A192F] text-[#FFFFFF] rounded-[8px] text-[13px] font-bold shadow-sm transition-all">
              <span className="material-symbols-outlined text-[20px]">local_shipping</span> Logística (CRUD)
            </Link>
            <Link href="/admin/destinos" className="flex items-center gap-3 px-3 py-2.5 text-[#475569] hover:text-[#0A192F] hover:bg-[#F7F9FB] rounded-[8px] text-[13px] font-bold transition-all">
       ...
       <div className="bg-[#FFFFFF] rounded-[12px] border border-[#D8DADC] shadow-sm flex flex-col overflow-hidden mb-12">
          <div className="p-6 border-b border-[#D8DADC] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#F7F9FB]">
            ...
            <form method="GET" className="flex gap-2 w-full sm:w-auto">
              ...
              {query && (
                <Link href="/viajes" className="bg-[#EF4444]/10 text-[#DC2626] border border-[#EF4444]/20 px-3 py-2 rounded-[6px] text-[12px] font-bold uppercase tracking-widest hover:bg-[#EF4444]/20 transition-colors flex items-center justify-center">
